...
######################################################################
# File Name: l4-6.sh (Thu Dec 25 08:00:00 2023);
######################################################################
set -o nounset
#bak=$IFS
#IFS=$'\n'
...
######################################################################
if [ "$hep" == "help" ]; then
	echo "Usage: l4-6.sh [-r reference genome] [-c ccs reads] [options]
Options:
	Input/Output:
	-r          reference genome
	-c          ccs reads
...
#=====================================================================
## step2: minimap2 qryHifi reads to ref, and get longAlt_ref.fa
#=====================================================================
...
#=====================================================================
## step3: minimap2 qryHifi reads to ref, and get ccsAlt_qry.fq
#=====================================================================
echo -e "Step3: minimap2 $ccsbase1 to ${refbase2}_racon${npr}.fa, then get ccsAlt_qry.fq!"
mkdir ccsAlt_qry
declare -i flag=0
cat ${refbase2}_racon${npr}To${ccsbase2}_m.bed | while read id start end
do
	flag=$(($flag + 1))
	echo -e "$id\t$start\t$end" > ccsAlt_qry/block_${flag}.bed
	awk 'ARGIND==1{a[$1];b[$1]=$2;c[$1]=$3} ARGIND==2{if($6 in a && $9>=b[$6] && $8<=c[$6] && $10/$2>=0.9 && $13~/tp:A:P/){print $1}}' ccsAlt_qry/block_${flag}.bed ${refbase2}_racon${npr}To${ccsbase2}.paf | sort | uniq > ccsAlt_qry/block_${flag}.id
	seqkit grep -f ccsAlt_qry/block_${flag}.id $ccsbase1 -j $thr > ccsAlt_qry/block_${flag}.fq
	rm ccsAlt_qry/block_${flag}.bed ccsAlt_qry/block_${flag}.id
done
...
#=====================================================================
echo -e "Step4: partial assembly with ccsAlt_qry.fq and longAlt_ref.fa!"
mkdir partial_assembly
flag=$(wc -l ${refbase2}_racon${npr}To${ccsbase2}_m.bed | awk '{print $1}')
for i in $(seq 1 $flag)
do
    hifiasm --primary -o partial_assembly/block_${i} --ul longAlt_ref/block_${i}.fa ccsAlt_qry/block_${i}.fq -t $thr
done
...
for i in $(seq 1 $flag)
do
	awk '/^S/{print ">"$2;print $3}' partial_assembly/block_${i}.p_ctg.gfa > longAlt_qry/block_${i}.p_ctg.fa
	awk '/^S/{print ">"$2;print $3}' partial_assembly/block_${i}.a_ctg.gfa > longAlt_qry/block_${i}.a_ctg.fa
	cat longAlt_qry/block_${i}.p_ctg.fa longAlt_qry/block_${i}.a_ctg.fa > longAlt_qry/block_${i}_pa.fa
	seqkit seq -n longAlt_qry/block_${i}_pa.fa -j $thr > longAlt_qry/block_${i}_pa.id
	rm longAlt_qry/block_${i}.p_ctg.fa longAlt_qry/block_${i}.a_ctg.fa
done
unset flag i

# 5.2 filter longAlt_qry.fa by reads depth
# module load samtools/1.11
flag=$(wc -l ${refbase2}_racon${npr}To${ccsbase2}_m.bed | awk '{print $1}')
for i in $(seq 1 $flag)
do
	minimap2 -ax map-hifi longAlt_qry/block_${i}_pa.fa ccsAlt_qry/block_${i}.fq -t $thr | samtools view -bS -@ $thr - | samtools sort -o longAlt_qry/block_${i}_s.bam -@ $thr -
	samtools depth -a longAlt_qry/block_${i}_s.bam > longAlt_qry/block_${i}_dep.txt
	awk '$3==0' longAlt_qry/block_${i}_dep.txt | awk '{print $1}' | sort | uniq > longAlt_qry/block_${i}_dep0.id
	awk 'ARGIND==1{a[$1]} ARGIND==2{if(!($1 in a)){print $1}}' longAlt_qry/block_${i}_dep0.id longAlt_qry/block_${i}_pa.id > longAlt_qry/block_${i}_pa_f1.id
	seqkit grep -f longAlt_qry/block_${i}_pa_f1.id longAlt_qry/block_${i}_pa.fa -j $thr >> longAlt_qry/block_pa_f1.fa
done
unset flag i

# 5.3 filter ontAlt_qry.fa by reads length
seqkit rename longAlt_qry/block_pa_f1.fa -j $thr | seqkit seq -m $dfl -j $thr > longAlt_qry/block_pa_f2.fa

ln -s longAlt_qry/block_pa_f2.fa longAlt_qry.fa
...
mv *racon* part* *longAlt_* block* $out
[[ $? -eq 0 ]] && echo -e "Congratulations! The l4-6.sh is done!\n"
