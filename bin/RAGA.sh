...
######################################################################
## File Name: RAGA.sh (Mon June 03 09:30:00 2024);
######################################################################
set -o nounset
#bak=$IFS
#IFS=$'\n'
...
if [ "$homo" == "homo" ]; then
	...
	echo -e "Initial Assembly: Assemble inbred/homozygous genomes."
	hifiasm -o contigs -l0 ../$ccsbase1 -t $thr
	[[ $? -eq 0 ]] || exit 1
	awk '/^S/{print ">"$2;print $3}' contigs.*p_ctg.gfa > contigs.bp.p_ctg.fa
	ln -s contigs.bp.p_ctg.fa initial.fa

elif [ "$hetero" == "hetero" ]; then
	# b. Assemble heterozygous genomes
	echo -e "Initial Assembly: Assemble heterozygous genomes."
	...
	echo -e "Initial Assembly: Assemble haplotype-resolved genomes with Hi-C reads."
...
#	echo -e "Initial Assembly: Assemble phased genomes with Trio binning."
...
#	hifiasm -o HG002.asm -t32 -1 pat.yak -2 mat.yak HG002-HiFi.fa.gz
	...
	# e. default assembly
	echo -e "The user did not provide the assembly parameter, so the default assembly mode 'homo' is used."
	hifiasm -o contigs -l0 ../$ccsbase1 -t $thr
	[[ $? -eq 0 ]] || exit 1
	awk '/^S/{print ">"$2;print $3}' contigs.*p_ctg.gfa > contigs.bp.p_ctg.fa
	ln -s contigs.bp.p_ctg.fa initial.fa
...
if [ "$homo" == "homo" ]; then
	# a. Assemble inbred/homozygous genomes
	echo -e "Optimized_assembly: Assemble inbred/homozygous genomes."
	hifiasm -o contigs --ul ../Alternative_reads/longAlt_sur.fa ../$ccsbase1 -t $thr
	[[ $? -eq 0 ]] || exit 1
	awk '/^S/{print ">"$2;print $3}' contigs.*p_ctg.gfa > contigs.bp.p_ctg.fa
	ln -s contigs.bp.p_ctg.fa RAGA-optimized.fa
#	ln -s contigs.bp.p_ctg.fa RAGA-optimized_contigs.fa

elif [ "$hetero" == "hetero" ]; then
	# b. Assemble heterozygous genomes
	echo -e "Optimized_assembly: Assemble heterozygous genomes."
	hifiasm -o contigs --ul ../Alternative_reads/longAlt_sur.fa ../$ccsbase1 -t $thr
	[[ $? -eq 0 ]] || exit 1
	awk '/^S/{print ">"$2;print $3}' contigs.*p_ctg.gfa > contigs.bp.p_ctg.fa
	ln -s contigs.bp.p_ctg.fa RAGA-optimized.fa
#	ln -s contigs.bp.p_ctg.fa RAGA-optimized_contigs.fa
	...
	# c. Generate a pair of haplotype-resolved assemblies with paired-end Hi-C reads
	echo -e "Optimized_assembly: Assemble haplotype-resolved genomes with Hi-C reads."
	hifiasm -o contigs --h1 ../$hic1base1 --h2 ../$hic2base1 --ul ../Alternative_reads/longAlt_sur.fa ../$ccsbase1 -t $thr
	[[ $? -eq 0 ]] || exit 1
	awk '/^S/{print ">"$2;print $3}' contigs.hic.hap1.p_ctg.gfa > contigs.hic.hap1.p_ctg.fa
	awk '/^S/{print ">"$2;print $3}' contigs.hic.hap2.p_ctg.gfa > contigs.hic.hap2.p_ctg.fa
	ln -s contigs.hic.hap1.p_ctg.fa RAGA-optimized_hap1.fa
	ln -s contigs.hic.hap2.p_ctg.fa RAGA-optimized_hap2.fa
...
#elif [ "$trio" == "trio" ]; then
#	d. Trio binning assembly
#	echo -e "Optimized_assembly: Assemble phased genomes with Trio binning."
#	yak count -b37 -t16 -o pat.yak <(cat pat_1.fq.gz pat_2.fq.gz) <(cat pat_1.fq.gz pat_2.fq.gz)
#	yak count -b37 -t16 -o mat.yak <(cat mat_1.fq.gz mat_2.fq.gz) <(cat mat_1.fq.gz mat_2.fq.gz)
#	hifiasm -o HG002.asm -t32 -1 pat.yak -2 mat.yak HG002-HiFi.fa.gz
