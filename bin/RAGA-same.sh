...
######################################################################
## File Name: RAGA-same.sh (Thu Jan 01 08:30:00 2024);
######################################################################
set -o nounset
#bak=$IFS
#IFS=$'\n'
...
######################################################################
if [ "$hep" == "help" ]; then
	echo "Usage: RAGA-same.sh [-r reference genome] [-q source genome] [-c ccs reads] [options]
Options:
	Input/Output:
	-r          reference genome
	-q          source assembly
...
See more information at https://github.com/wzxie/RAGA.
	...
	[[ $qry == "" ]] && echo -e "ERROR: Path to source assembly not found, assign using -q." && exit
