...
######################################################################
## File Name: same.sh (Wed June 05 18:00:00 2024);
######################################################################
set -o nounset
#bak=$IFS
#IFS=$'\n'
...
# c. Result return
if [ "$hep" == "help" ]; then
	echo "Usage: same.sh [-r reference genome] [-q source assembly] [-c source PacBio HiFi reads] [options]
...
if [ "$solo" != "yes" ]; then
	[[ $? -eq 0 ]] && echo -e "Congratulations! The same.sh is done.\n"
fi
