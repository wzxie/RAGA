...
######################################################################
# File Name: diff.sh (Wed June 11 22:00:00 2024);
######################################################################
set -o nounset
#bak=$IFS
#IFS=$'\n'
...
# c. Result return
if [ "$hep" == "help" ]; then
	echo "Usage: diff.sh [-r reference genome] [-c source PacBio HiFi reads] [options]
Options:
	Input/Output:
	-r          reference genome
...
if [ "$solo" != "yes" ]; then
	[[ $? -eq 0 ]] && echo -e "Congratulations! The diff.sh is done.\n"
fi
